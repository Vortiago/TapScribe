...
// view-model the popup's meeting card renders. The analogue of the Windows
// tray Bridge's pure `StatusView` — no fetch, no DOM, no storage, no clock,
// so it is exhaustively unit-tested with plain inputs and no mocks.
//
// An ES module, imported by the popup only — the content script doesn't render
// a card, so it never imports this (and `control-client.js` stays a classic
// global because the content script DOES need it in both worlds).
//
...
export function progressLabel(raw) {
...
/** @param {PipelinePoll | null | undefined} raw @returns {string} */
export function failureReason(raw) {
...
// The view-model shape the card renders. Every field is always present so the
// renderer never has to feature-detect: irrelevant fields are null.
...
// Every informative poll state (running / done / failed) is resolved from the
// body alone, so those branches are pure functions of the response.
/**
 * @param {PipelinePoll | null | undefined} [raw]
 * @param {{ meetingActive?: boolean, ending?: boolean }} [opts]
 * @returns {PipelineView}
 */
export function map(raw, opts) {
  const o = opts || {};
