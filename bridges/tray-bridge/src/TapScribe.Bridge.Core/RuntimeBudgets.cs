...
public sealed record RuntimeBudgets
{
    ...
    /// <summary>
    /// How long the open phase's pre-flight may take before it gives up — the session mint for
    /// a meeting, the connection probe for an attached tap. Without a bound HttpClient waits
    /// its 100 s default, which would wedge the shell on "Starting…" or "Connecting…" against a
    /// host that accepts the connection and then never replies: exactly the case a test has to
    /// be able to reach without spending the real budget.
    /// </summary>
    public TimeSpan PreflightTimeout { get; init; } = TimeSpan.FromSeconds(20);

    /// <summary>
    /// How long teardown waits for a Start still in flight to reach the point where it can
    /// tear its own meeting down. A backstop, not a promise: the pre-flight it is usually
