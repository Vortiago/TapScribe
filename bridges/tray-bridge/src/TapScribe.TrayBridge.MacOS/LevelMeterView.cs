...
/// <summary>
/// A display-only level bar with a marker at the gate's open threshold: the Mac sibling of
/// WinForms' <c>LevelMeterBar</c> (#421).
///
/// Both numbers are RMS and both go through Core's <see cref="LevelMeterScale"/>, so the bar and
/// the gate agree what "half way" means. That is the marker's whole point: the operator drags
/// Sensitivity until it sits under where their speech peaks.
/// </summary>
internal sealed class LevelMeterView : NSView
{
    ...
    // Repaint only on a visible change: both are set ~15 times a second while the window is up.
    ...
    public override void DrawRect(CGRect dirtyRect)
    {
        ...
        // Two greens: below the threshold the gate is shut, so the fill reads "heard, not
        // recorded". Open or shut is Core's call, because the WinForms bar asks it too.
        if (fill > 0)
        {
