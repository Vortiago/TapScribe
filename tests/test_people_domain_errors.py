...
People CRUD routes collapse to thin shims over `DOMAIN_ERROR_STATUS`.
...
domain errors in `routes.errors.DOMAIN_ERROR_STATUS`.
...
def test_people_domain_errors_carry_no_status_code_attribute(exc_type) -> None:
    """#228's rule: the HTTP status lives in `DOMAIN_ERROR_STATUS` and nowhere
...
def _route_defs() -> dict[str, ast.AST]:
    from tapscribe.routes import people as people_routes

    tree = ast.parse(Path(people_routes.__file__).read_text(encoding="utf-8"))
    return {
        node.name: node
...
def test_registering_the_people_errors_keeps_every_earlier_one() -> None:
    """This slice ADDS to `DOMAIN_ERROR_STATUS`. Rewriting the literal instead
    ...
    assert DOMAIN_ERROR_STATUS[SessionBusy] == 409
    assert DOMAIN_ERROR_STATUS[SessionNotFound] == 404
    assert DOMAIN_ERROR_STATUS[UnknownSource] == 400
