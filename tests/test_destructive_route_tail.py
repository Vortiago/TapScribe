...
`SessionDeleteError -> 500` registration that #228 put in `DOMAIN_ERROR_STATUS`
...
from tapscribe.routes.guards import ops_log
...
def _func_defs() -> dict[str, ast.AST]:
    """Every handler def across the modules these routes live in.

    The router split (#229 / PR #393) moved the HTTP surface out of `app.py`
    into per-resource modules, so the four destructive routes now sit in two of
    them — plus `strip.py`, which holds the sibling this slice must NOT sweep.
    """
    from tapscribe.routes import sessions as session_routes
    from tapscribe.routes import strip as strip_routes
    from tapscribe.routes import wav as wav_routes

    found: dict[str, ast.AST] = {}
    for mod in (session_routes, wav_routes, strip_routes):
...
def test_session_delete_uses_the_domain_error_seam() -> None:
    ...
    `DOMAIN_ERROR_STATUS`; the route should use it rather than translating by
...
def test_session_delete_failure_is_a_500_that_says_what_failed(
    ...
    `HTTPException` or as `SessionDeleteError` through `DOMAIN_ERROR_STATUS`,
...
# inline `print`, which a bare `def ops_log(m): print(m)` satisfies while
