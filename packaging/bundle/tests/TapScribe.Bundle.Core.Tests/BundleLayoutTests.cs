...
///
/// The fixture roots are normalised once, through <see cref="FixturePath"/>, rather than
/// at each expectation: <see cref="Path.GetFullPath"/> is idempotent, so a normalised root
/// survives the layout unchanged and an assertion names it directly. Wrapping per
/// assertion instead leaves every future one needing to remember the wrapper.
/// </summary>
public class BundleLayoutTests
{
    ...
    private static readonly string Program = FixturePath.Rooted("/opt/prog");
    private static readonly string Profile = FixturePath.Rooted("/home/op");
    private static readonly string MacRoot = FixturePath.Rooted(MacPayload);
    private static readonly string MacHome = FixturePath.Rooted("/Users/op");

    private static BundleLayout Layout(string? program = null, string? profile = null) =>
        BundleLayout.ForWindows(program ?? Program, profile ?? Profile);

    private static BundleLayout MacLayout(string version = "1.3.0") =>
        BundleLayout.ForMacOS(MacRoot, MacHome, version);
    ...
    public void Resolve_PutsTheEmbeddedInterpreterUnderTheProgramDirectory()
    {
        ...
        Assert.Equal(Path.Join(Program, "python"), layout.PythonDirectory);
        Assert.Equal(Path.Join(Program, "python", "python.exe"), layout.Python);
        Assert.Equal(Path.Join(Program, "python", "pythonw.exe"), layout.Pythonw);
        Assert.Equal(Path.Join(Program, "wheel"), layout.WheelDirectory);
    }
    ...
    public void Resolve_KeepsOperatorDataOutsideTheProgramDirectory()
    {
        ...
        Assert.Equal(Path.Join(Profile, "TapScribe"), layout.DataDirectory);
    ...
    public void Resolve_PointsAtTheRecordersOwnAuthPasswordFile()
    {
        ...
        Assert.Equal(Path.Join(Profile, "TapScribe", ".auth-password"), Layout().PasswordFile);
    }
    ...
    public void Resolve_PutsTheHostLogInItsOwnFolderUnderTheDataDirectory()
    {
        ...
        Assert.Equal(Path.Join(Profile, "TapScribe", "logs"), layout.LogDirectory);
    ...
    public void ForMacOS_PutsTheDataRootUnderApplicationSupport()
    {
        ...
        Assert.Equal(
            Path.Join(MacHome, "Library", "Application Support", "TapScribe"),
    ...
    public void ForMacOS_ReadsThePayloadFromInsideTheApp()
    {
        ...
        Assert.Equal(Path.Join(MacRoot, "python"), layout.PayloadPythonDirectory);
        Assert.Equal(Path.Join(MacRoot, "wheel"), layout.PayloadWheelDirectory);
    }
    ...
    public void MacOSPayload_ClimbsToContentsResourcesFromWhereverTheAssembliesSit()
    {
        ...
        string expected = Path.Join(FixturePath.Rooted("/Applications/TapScribe.app"), "Contents", "Resources");
