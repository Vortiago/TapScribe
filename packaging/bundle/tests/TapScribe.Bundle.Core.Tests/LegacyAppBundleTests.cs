...
public class LegacyAppBundleTests
{
    ...
    {
        // ContainingApp walks up from Path.GetFullPath(baseDir), so the expectation is
        // normalised the same way (FixturePath says why). The null case asserts "no .app
        // anywhere above" and has no path to normalise, which RootedOrNull handles.
        Assert.Equal(FixturePath.RootedOrNull(expected), LegacyAppBundle.ContainingApp(baseDir));
    }
